...
Lemma exists_equiv X (P Q : X -> Prop) : 
(forall x, P x <-> Q x) -> ex P <-> ex Q.
Proof.
  split.
  intro.
  destruct H0.
...
Section extra_list_results.
  ...
  *)
  ...
  Lemma cons_inj a l1 b l2 : a::l1 = b::l2 -> a = b /\ l1 = l2.
  Proof. inversion 1; split; trivial. Qed.
  ...
  Lemma app_split l1 l2 r1 r2 : 
       ...
       -> exists m, l2 = l1++m /\ r1 = m++r2
                 \/ l1 = l2++m /\ r2 = m++r1.
  Proof.
    revert l2. induction l1 as [ | a l1 IH1 ].
    + simpl.
      intros l2 H2.
      exists l2; left; split; trivial.
    + intros [ | b l2 ] H2. simpl in H2.
      * exists (a::l1); right; simpl. rewrite H2. split; trivial.
      * apply cons_inj in H2.
        destruct H2 as [ E2 H2 ].
        subst b.
        destruct IH1 with (1 := H2)
          as (m & [ (H3 & H4) | (H3 & H4) ]).
        - exists m; left. simpl; split; trivial. f_equal. exact H3.
        - exists m; right. simpl. split; trivial. f_equal. exact H3.
  Qed.
  ...
  Section list_rev_ind.
    ...
    Proof.
      ...
      apply P_rev. (* trivial marche aussi *)
    Qed.
...
Section strict_lists.
  ...
  Proof.
    ...
    + intros. apply H.
      ...
      * apply H0.
  + intros H1 z y H2 H3.
      destruct H2; destruct H; apply H1; apply H3.
  Qed.
  ...
  Proof.
    ...
    + intros. apply H.
      ...
      * simpl. left. trivial.
  + intros H1 z y H2 H3.
    destruct H3; destruct H; apply H1; apply H2. 
  Qed.
  ...
  Proof.
    ...
    + split.
      ...
      * intros x y H1 H2. apply H.
        ...
        - exact H2.
 + intros H x y H1 H2. 
    destruct H. apply in_app_iff in H1. destruct H1.  
    *  apply H.
      -  exact H1.
      - exact H2.
    *  apply H0.  
  ...
  split.
    ...
    + intros H x y H1 H2. destruct H. apply in_app_iff in H2. destruct H2.  
        * apply H.
          - exact H1.
          -  exact H2.
        * apply H0.
  ...
  Proof.
    ...
    split.
    * exact H0.
    *  apply slu_app_l in H. destruct H. exact H.
  Qed.
